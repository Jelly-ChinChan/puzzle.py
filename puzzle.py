# streamlit_app.py —— 內建題庫 + 模式2/3選項配對顯示完成版
...
# 註：這份內建清單涵蓋你檔案裡的單字句（如需擴充可直接在此新增 dict）
...
def get_options_for_q(qidx, mode):
    ...
    if mode == MODE_2:  # 中文選項（值＝英文）
        ...
        for zh in display:
            ...
            value.append(str(en_match).strip())
...
if st.session_state.round:
    ...
    if st.session_state.submitted and st.session_state.records:
        ...
        zh2en = { (it.get("meaning_zh") or "").strip(): it["answer_en"].strip() for it in QUESTION_BANK }
